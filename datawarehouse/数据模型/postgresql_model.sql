...
CREATE TABLE IF NOT EXISTS marketing_activity_record (
    ...
    user_id VARCHAR(50) NOT NULL COMMENT '用户ID',
    product_id VARCHAR(50) COMMENT '商品ID',
    channel_id VARCHAR(2) COMMENT '渠道ID (010:抖音, 020:微信小程序, 021:微信视频号, 030:淘宝, 031:天猫, 040:线下合作, 099:其他)',
    business_line_code VARCHAR(3) COMMENT '业务线编码 (101:核心IP产品, 102:场景化旅游商品, 103:引流品/虚拟服务)',
    source_id VARCHAR(2) COMMENT '流量来源ID (001:自然流量, 002:付费流量, 003:内容流量, 004:社交裂变, 005:线下引流, 099:未知/其他)',
...
-- 营销活动记录表索引
CREATE INDEX idx_activity_record_business_line ON marketing_activity_record(business_line_code);
CREATE INDEX idx_activity_record_source ON marketing_activity_record(source_id);

-- 评价反馈表索引
CREATE INDEX idx_review_business_line ON product_review(business_line_code);

-- 会员积分表索引
CREATE INDEX idx_points_business_line ON member_points(business_line_code);
CREATE INDEX idx_points_channel ON member_points(channel_id);

-- 退款/售后表索引
CREATE INDEX idx_refund_business_line ON refund_aftersales(business_line_code);
